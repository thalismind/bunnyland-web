...
export function WorldEditorPage() {
  ...
  const [apiUrl, setApiUrl] = useState('/api/'); const [status, setStatus] = useState<Status>({ kind: '', text: 'Ready' });
  ...
  const fetchSnapshot = async () => {
    ...
    try {
      ...
      try { schemaRef.current = await api.sendJson(base, '/play/world/schema'); } catch { schemaRef.current = null; }
      try { const data = await api.sendJson(base, '/play/world/library'); fragmentsRef.current = normalizeFragments(data, data.library_id || 'server'); } catch { fragmentsRef.current = []; }
