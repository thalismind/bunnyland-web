...
export function EditorLayout(props: EditorLayoutProps) {
  ...
  return <div id="main" class={`app-grid${snapshotVisible ? '' : ' snapshot-hidden'}`}>
    <Pane title="Entities" tools={<><span class="pane-count" id="entity-count">{entities.length}/{Object.keys(world.entities).length}</span><Button disabled={props.busy} id="btn-add-entity" onClick={props.onAddEntity}>Add</Button><Button id="btn-delete-entity" disabled={!selected || props.busy} onClick={props.onDeleteEntity}>Delete</Button></>}>
      <div class="pane-body"><input aria-label="Search world entities" type="text" id="entity-search" placeholder="find, type:, component:, invalid" spellcheck={false} autocomplete="off" value={props.search} onInput={event => props.onSearch(event.currentTarget.value)} /><div aria-label="World entities" id="entity-list" role="listbox"><EntityList entities={entities.map(entity => ({ id: entity.id, icon: entityIcon(entity), invalid: hasInvalidTarget(entity, world), name: entityDisplayName(entity), type: entityType(entity) }))} selectedId={selected?.id || null} onSelect={props.onSelect} /></div></div>
    </Pane>
    ...
    {snapshotVisible && <Pane id="preview-pane" title="Snapshot JSON" tools={<span class="pane-count" id="json-size">{new Blob([jsonText]).size} bytes</span>}>
